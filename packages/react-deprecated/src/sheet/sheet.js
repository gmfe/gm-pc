...
import { Pagination, Loading, Flex } from '@gm-pc/react'
import PaginationText from './pagination_text'
    ...
    _.each(list, (value, index) => {
      trs.push(
        <tr {...this.props.getTrProps(index)} key={index}>
          ...
          {actions ? <td className='text-center'>{actions.props.children(value, index)}</td> : null}
        </tr>
      )
  ...
  render() {
    ...
    if (select && list.length > 0) {
      ...
      isSelectAll =
        ...
        !_.find(list, (value) => !select.props.isDisabled(value) && !value._gm_select)
    }
    // 检查是否有某一列收起
    ...
    return (
      <div className={classNames('gm-sheet', this.props.className)}>
        {select && batchs ? (
          <div className='gm-margin-bottom-5 text-right'>{batchs.props.children}</div>
        ) : null}
